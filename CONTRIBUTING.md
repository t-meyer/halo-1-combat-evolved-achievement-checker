...
1. **Confirm the bonus mission ids.** `e10`, `e20` and `e30` are Boarding
   ...
   of them, run `.\blamscan.ps1 -Raw`, and see which id gained a completion
...
4. **Unrecognised tags.** If blamscan prints an "Unrecognised tags" block after
...
- **Never commit save files.** `.gitignore` covers `saves/`, `samples/`, `*.sav`
  ...
  played on that PC.
- **Redact before posting.** XUIDs and gamertags appear in the blamscan header
...
- **Mission data belongs in `data/missions.json`**, not in the script. The table
  inside `blamscan.ps1` is only a fallback for when the JSON file is missing;
...
```powershell
.\blamscan.ps1            # against a save known to be at 100%
```
...
- the output of `.\blamscan.ps1 -Raw` with XUID and gamertag removed
